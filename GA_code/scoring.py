...
def run_calculations(polymer, unit_list):
    """
    ...
    if not exists:
        ...
        subprocess.call('(mkdir %s_xtb)' % (filename), shell=True)
        
        #subprocess.call('(cd ../input_mol && sbatch -J ../input_mol/%s.mol --export=run_label=%s,gen_counter=%s,smiles=%s GA_cron/GA_run_GFN2.slurm)' %(filename, run_label, gen_counter, poly_smiles), shell=True)
        #subprocess.call('sbatch -J %s --export=run_label=%s,gen_counter=%s /ihome/ghutchison/dch45/ga_methods/github/GA-Best-Practices/GA_cron/GA_run_GFN2.slurm' %(filename, run_label, gen_counter), shell=True)
        
        #TODO: change this line to run your calculation
        # run xTB geometry optimization
        subprocess.call('(cd %s_xtb && /ihome/ghutchison/oda6/xtb/xtb-641/bin/xtb ../../input_mol/%s.mol --opt > ../../output_GFN2/%s.out)' % (filename, filename, filename), shell=True)
        # compress the output file
        subprocess.call('(gzip ../output_GFN2/%s.out)' % filename, shell=True)
        # copy optimized geometry to main optimized geom directory
        subprocess.call('(cp %s_xtb/xtbopt.mol ../opt_mol/%s_opt.mol)' % (filename, filename), shell=True)
        # delete xtb run directory for the polymer
        subprocess.call('(rm -r %s_xtb)' % (filename), shell=True)
...
def fitness_function(population, unit_list):
    ...
    for x in range(len(population)):
        ...
        run_calculations(polymer, unit_list)
